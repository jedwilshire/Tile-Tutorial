import pygame, settings, math, os
...
class Zombie(pygame.sprite.Sprite):
    def __init__(self, game, x, y):
        super().__init__()
        self.game = game 
        self.non_rotated_image = pygame.image.load(os.path.join(game.images_folder, settings.ZOMBIE_IMG_FILE)).convert_alpha()
        #self.non_rotated_image = pygame.transform.scale(self.non_rotated_image, (settings.TILE_SIZE, settings.TILE_SIZE))
        self.image = self.non_rotated_image
        self.rect = self.image.get_rect()
        self.pos = pygame.math.Vector2(x, y) * settings.TILE_SIZE
        ...
        self.vel = pygame.math.Vector2(settings.ZOMBIE_SPEED, 0)
        # angle of rotatoin
        self.angle = 0
        self.game.sprite_group.add(self)
    ...
    def update(self):
        ...
        # rotate velocity vector to match angle of rotation
        self.vel = pygame.math.Vector2(settings.ZOMBIE_SPEED, 0).rotate(-self.angle)
...
class Wall(pygame.sprite.Sprite):
    def __init__(self, game, x, y):
        super().__init__()
        self.game = game
        self.image = pygame.image.load(os.path.join(game.images_folder, settings.WALL_IMG_FILE)).convert_alpha()
        self.image = pygame.transform.scale(self.image, (settings.TILE_SIZE, settings.TILE_SIZE))
        self.rect = self.image.get_rect()
