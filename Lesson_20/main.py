...
class Application:
    ...
    def load_game(self):
        ...
        for item in self.map.tmxdata.objects:
            ...
            elif item.name in settings.ITEM_IMGS.keys():
    ...
    def load_images(self):
        ...
        self.flash_images = []
        for img in settings.FLASH_IMGS:
            f_img = pygame.image.load(os.path.join(self.images_folder, img)).convert_alpha()
            f_img = pygame.transform.scale(f_img, (settings.FLASH_SIZE, settings.FLASH_SIZE))
            self.flash_images.append(f_img)
            
        self.item_images = {}
        for item in settings.ITEM_IMGS.keys():
            img_file = os.path.join(self.images_folder, settings.ITEM_IMGS[item])
    ...
    def game_update(self):
        ...
        for item in item_hits:
            if item.item_type == 'health' and self.player.health < settings.PLAYER_HEALTH:
                self.player.use_health_pack(settings.HEALTH_PACK_AMT)
                item.kill()
