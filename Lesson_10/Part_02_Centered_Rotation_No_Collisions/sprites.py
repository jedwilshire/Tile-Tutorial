...
class Player(pygame.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        """
                    IMPORTANT
            TO USE CENTERED ROTATION, CHANGES MUST BE MADE TO TILE MAP TO
            PREVENT BOBBING CAMERA WHILE ROTATING
        """
        ...
        if direction == 'x':
            ...
            if len(wall) != 0:
                if self.vel.x > 0: # moving right pos.x is left side of player
                    ...
                    self.pos.x = wall[0].rect.left - self.rect.width / 2.0
                if self.vel.x < 0: # moving left
                    # set pos.x to right side of wall
                    self.pos.x = wall[0].rect.right + self.rect.width / 2.0
        ...
        # check for y direction in same manner
            ...
            if len(wall) != 0:
                if self.vel.y > 0: # moving down pos.y is top of player
                    ...
                    self.pos.y = wall[0].rect.top - self.rect.height / 2.0
                if self.vel.y < 0: # moving up
                    # set pos.y to bottom of wall
                    self.pos.y = wall[0].rect.bottom + self.rect.height / 2.0
                self.vel.y = 0
